...
$verboseLogFile = $ScriptDirectory+"\Atelier3.log"
...
$Client2 = get-VM -Name G*_Linux
Write-Host "---------- Atelier N°3 Start ----------" -ForegroundColor Green
write-host ""
# Shutdown Party #
...
foreach ($item in $Server.name) 
    {
        ...
        {
           New-Logtrace "The VM $item is PoweredOn Try to shutdown"
...
foreach ($item in $ESXI.name) 
    {
        New-Logtrace "Starting the ESXI $item"
        start-vm  $item | Out-Null
        Wait-Tools -VM $item | Out-Null
        New-Logtrace "The ESXI $item is ready"
    }
    foreach ($item in $Client.name) 
    {
        ...
        start-vm  $item | Out-Null
        Wait-Tools -VM $item | Out-Null
        New-Logtrace "The VM $item is ready"
    }
...
New-Logtrace "Atelier N°3 Starting in complete"
